...
class Game:
    ...
    pygame.mixer.init()
    pygame.mixer.music.load('./asset/somjogo.wav')
    ...
    def run(self):
        while True:
            ...
            if self.rock_x < self.boar_x and not self.passed:
                self.score += 1
                self.passed = True


            if self.rock_x < -self.rock.get_width():
                self.rock_x = WIN_WIDTH + 50
                self.passed = False
                self.rock = random.choice([self.rock_small, self.rock_big])
                self.rock_y = self.ground - self.rock.get_height()
            ...
            self.window.blit(score_text, (50, 50))
